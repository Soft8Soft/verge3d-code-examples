 ...
 */
    ...
    class TDSLoader extends v3d.Loader {
   ...
   * @return {Group} v3d.Group loaded from 3ds file.
   */


        parse(arraybuffer, path) {

            this.group = new v3d.Group();
            this.materials = [];
            this.meshes = [];
        ...
        readMaterialEntry(chunk, path) {
            ...
            while (next) {
                ...
                if (next.id === MAT_NAME) {
                    ...
                    material.blending = v3d.AdditiveBlending;

                } else if (next.id === MAT_DIFFUSE) {

                    this.debugMessage('   Diffuse v3d.Color');
                    material.color = this.readColor(next);

                } else if (next.id === MAT_SPECULAR) {

                    this.debugMessage('   Specular v3d.Color');
        ...
        readMesh(chunk) {
            ...
            while (next) {
                ...
                if (next.id === POINT_ARRAY) {
                    ...
                    const matrix = new v3d.Matrix4(); //X Line
   ...
   * @param {Chunk} chunk Chunk in use.
   * @param {Mesh} mesh v3d.Mesh to be filled with the data read.
   */
        ...
        readFaceArray(chunk, mesh) {
            ...
            while (! chunk.endOfChunk) {
                ...
                if (subchunk.id === MSH_MAT_GROUP) {
                    ...
                    this.debugMessage('      Material v3d.Group');
   ...
   */
        ...
        readMap(chunk, path) {
            ...
            let next = chunk.readChunk();
            let texture = {};
            const loader = new v3d.TextureLoader(this.manager);
   ...
   * @return {Color} v3d.Color value read..
   */
        ...
        readColor(chunk) {
            ...
            if (subChunk.id === COLOR_24 || subChunk.id === LIN_COLOR_24) {
                ...
                this.debugMessage('      v3d.Color: ' + color.r + ', ' + color.g + ', ' + color.b);
