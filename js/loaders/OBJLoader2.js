...
if (v3d.OBJLoader2 === undefined) { v3d.OBJLoader2 = {} }

if (v3d.LoaderSupport === undefined) console.error('"v3d.LoaderSupport" is not available. "v3d.OBJLoader2" requires it. Please include "LoaderSupport.js" in your HTML.');

/**
 * Use this class to load OBJ data from files or to parse OBJ data from an arraybuffer
 * @class
 *
 * @param {v3d.DefaultLoadingManager} [manager] The loadingManager for the loader to use. Default is {@link v3d.DefaultLoadingManager}
 */

v3d.OBJLoader2 = function(manager) {
    console.info('Using v3d.OBJLoader2 version: ' + v3d.OBJLoader2.OBJLOADER2_VERSION);
    ...
    this.workerSupport = new v3d.LoaderSupport.WorkerSupport();
    this.terminateWorkerOnLoad = true;
};

v3d.OBJLoader2.OBJLOADER2_VERSION = '2.5.0';

v3d.OBJLoader2.prototype = {

    constructor: v3d.OBJLoader2,
    ...
    setModelName: function(modelName) {
        this.modelName = v3d.LoaderSupport.Validator.verifyInput(modelName, this.modelName);
    },
    ...
    setPath: function(path) {
        this.path = v3d.LoaderSupport.Validator.verifyInput(path, this.path);
    },
     ...
     *
     * @param {v3d.Object3D} streamMeshesTo Object already attached to scenegraph where new meshes will be attached to
     */
    ...
    /**
     * Set materials loaded by MTLLoader or any other supplier of an Array of {@link v3d.Material}.
     *
    ...
    /**
     * Instructs loaders to create indexed {@link v3d.BufferGeometry}.
     *
    ...
    onProgress: function(type, text, numericalValue) {
        var content = v3d.LoaderSupport.Validator.isValid(text) ? text: '';
        var event = {
            detail: {
                type: type,
        ...
        if (v3d.LoaderSupport.Validator.isValid(this.callbacks.onProgress)) this.callbacks.onProgress(event);
    ...
    _throwError: function(errorMessage) {
        if (v3d.LoaderSupport.Validator.isValid(this.callbacks.onReportError))  {
    ...
    load: function(url, onLoad, onProgress, onError, onMeshAlter, useAsync) {
        var resource = new v3d.LoaderSupport.ResourceDescriptor(url, 'OBJ');
    ...
    _loadObj: function(resource, onLoad, onProgress, onError, onMeshAlter, useAsync) {
        var scope = this;
        if (!v3d.LoaderSupport.Validator.isValid(onError)) {
        ...
        // fast-fail
        if (!v3d.LoaderSupport.Validator.isValid(resource)) onError('An invalid ResourceDescriptor was provided. Unable to continue!');
        ...
        // fast-fail
        if (!v3d.LoaderSupport.Validator.isValid(resource.url) || v3d.LoaderSupport.Validator.isValid(resource.content)) {

            fileLoaderOnLoad(v3d.LoaderSupport.Validator.isValid(resource.content) ? resource.content : null);

        } else {

            if (!v3d.LoaderSupport.Validator.isValid(onProgress)) {
     ...
     *
     * @param {v3d.LoaderSupport.PrepData} prepData All parameters and resources required for execution
     * @param {v3d.LoaderSupport.WorkerSupport} [workerSupportExternal] Use pre-existing WorkerSupport
     */
    ...
    parse: function(content) {
        ...
        if (!v3d.LoaderSupport.Validator.isValid(content)) {
        ...
        var parser = new v3d.OBJLoader2.Parser();
    ...
    parseAsync: function(content, onLoad) {
        ...
        var buildCode = function(codeSerializer) {
            ...
            workerCode += '  */\n\n';
            workerCode += 'v3d = { LoaderSupport: {}, OBJLoader2: {} };\n\n';
            workerCode += codeSerializer.serializeObject('v3d.LoaderSupport.Validator', v3d.LoaderSupport.Validator);
            workerCode += codeSerializer.serializeClass('v3d.OBJLoader2.Parser', v3d.OBJLoader2.Parser);

            return workerCode;
        };
        this.workerSupport.validate(buildCode, 'v3d.OBJLoader2.Parser');
    ...
    loadMtl: function(url, content, onLoad, onProgress, onError, crossOrigin, materialOptions) {
        var resource = new v3d.LoaderSupport.ResourceDescriptor(url, 'MTL');
    ...
    _loadMtl: function(resource, onLoad, onProgress, onError, crossOrigin, materialOptions) {
        if (v3d.MTLLoader === undefined) console.error('"v3d.MTLLoader" is not available. "v3d.OBJLoader2" requires it for loading MTL files.');
        if (v3d.LoaderSupport.Validator.isValid(resource) && this.logging.enabled) console.time('Loading MTL: ' + resource.name);

        var materials = [];
        var scope = this;
        var processMaterials = function(materialCreator) {
            var materialCreatorMaterials = [];
            if (v3d.LoaderSupport.Validator.isValid(materialCreator)) {
            ...
            if (v3d.LoaderSupport.Validator.isValid(resource) && scope.logging.enabled) console.timeEnd('Loading MTL: ' + resource.name);
            onLoad(materials, materialCreator);
        };

        // fast-fail
        if (!v3d.LoaderSupport.Validator.isValid(resource) || (! v3d.LoaderSupport.Validator.isValid(resource.content) && ! v3d.LoaderSupport.Validator.isValid(resource.url))) {

            processMaterials();

        } else {

            var mtlLoader = new v3d.MTLLoader(this.manager);
            crossOrigin = v3d.LoaderSupport.Validator.verifyInput(crossOrigin, 'anonymous');
            mtlLoader.setCrossOrigin(crossOrigin);
            mtlLoader.setResourcePath(resource.resourcePath || resource.path);
            if (v3d.LoaderSupport.Validator.isValid(materialOptions)) mtlLoader.setMaterialOptions(materialOptions);
            ...
            var parseTextWithMtlLoader = function(content) {
                ...
                if (typeof(content) !== 'string' && ! (content instanceof String)) {
                    ...
                    if (content.length > 0 || content.byteLength > 0) {
                        ...
                        contentAsText = v3d.LoaderUtils.decodeText(content);
            ...
            } else if (v3d.LoaderSupport.Validator.isValid(resource.url)) {
 ...
 * @class
 */
v3d.OBJLoader2.Parser = function() {
...
v3d.OBJLoader2.Parser.prototype = {

    constructor: v3d.OBJLoader2.Parser,
    ...
    setMaterials: function(materials) {
        this.materials = v3d.LoaderSupport.Validator.verifyInput(materials, this.materials);
        this.materials = v3d.LoaderSupport.Validator.verifyInput(this.materials, {});
    },

    setCallbackMeshBuilder: function(callbackMeshBuilder) {
        if (!v3d.LoaderSupport.Validator.isValid(callbackMeshBuilder)) {
    ...
    checkSubGroup: function() {
        ...
        if (!v3d.LoaderSupport.Validator.isValid(this.rawMesh.subGroupInUse)) {
    ...
    buildFace: function(faceIndexV, faceIndexU, faceIndexN) {
        ...
        if (this.useIndices) {
            ...
            if (v3d.LoaderSupport.Validator.isValid(indicesPointer)) {
    ...
    processCompletedMesh: function() {
        ...
        if (v3d.LoaderSupport.Validator.isValid(result)) {
    ...
    buildMesh: function(result) {
        ...
        var haveVertexColors = v3d.LoaderSupport.Validator.isValid(colorFA);
        ...
        for (var oodIndex in meshOutputGroups) {
            ...
            if (!v3d.LoaderSupport.Validator.isValid(materialOrg) && ! v3d.LoaderSupport.Validator.isValid(material)) {
            ...
            if (!v3d.LoaderSupport.Validator.isValid(material)) {
            ...
            if (this.logging.enabled && this.logging.debug) {
                var materialIndexLine = v3d.LoaderSupport.Validator.isValid(selectedMaterialIndex) ? '\n\t\tmaterialIndex: ' + selectedMaterialIndex : '';
        ...
        this.callbackMeshBuilder(
            ...
            [vertexFA.buffer],
            v3d.LoaderSupport.Validator.isValid(indexUA) ? [indexUA.buffer] : null,
            v3d.LoaderSupport.Validator.isValid(colorFA) ? [colorFA.buffer] : null,
            v3d.LoaderSupport.Validator.isValid(normalFA) ? [normalFA.buffer] : null,
            v3d.LoaderSupport.Validator.isValid(uvFA) ? [uvFA.buffer] : null
        );
    },
