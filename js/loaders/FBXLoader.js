...
(function() {
     ...
     * a v3d.Group.
     * @param {v3d.LoadingManager} manager - Loading Manager for loader to use.
     */
    v3d.FBXLoader = function(manager) {

        this.manager = (manager !== undefined) ? manager : v3d.DefaultLoadingManager;

    };

    Object.assign(v3d.FBXLoader.prototype, {

        /**
         * Loads an ASCII/Binary FBX file from URL and parses into a v3d.Group.
         * v3d.Group will have an animations property of AnimationClips
         * of the different animations exported with the FBX.
         * @param {string} url - URL of the FBX file.
         * @param {function(v3d.Group):void} onLoad - Callback for when FBX file is loaded and parsed.
        ...
        load: function(url, onLoad, onProgress, onError) {
            ...
            var resourceDirectory = v3d.Loader.prototype.extractUrlBase(url);
        ...
        /**
         * Parses an ASCII/Binary FBX file and returns a v3d.Group.
         * v3d.Group will have an animations property of AnimationClips
        ...
        parse: function(FBXBuffer, resourceDirectory) {
            ...
            var textures = parseTextures(FBXTree, new v3d.TextureLoader(this.manager).setPath(resourceDirectory), images, connections);
     ...
     * @param {Map<number, v3d.Texture>} textureMap
     * @param {Map<number, {parents: {ID: number, relationship: string}[], children: {ID: number, relationship: string}[]}>} connections
     * @returns {Map<number, v3d.Material>}
     */
    ...
    /**
     * Takes information from Material node and returns a generated v3d.Material
     * @param {FBXMaterialNode} materialNode
     * @param {Map<number, v3d.Texture>} textureMap
     * @param {Map<number, {parents: {ID: number, relationship: string}[], children: {ID: number, relationship: string}[]}>} connections
     * @returns {v3d.Material}
     */
    function parseMaterial(materialNode, textureMap, connections) {
        ...
        switch (type.toLowerCase()) {
            ...
            case 'lambert':
                ...
                break;
            default:
                console.warn('v3d.FBXLoader: No implementation given for material type %s in FBXLoader.js. Defaulting to standard material.', type);
                material = new v3d.MeshStandardMaterial({ color: 0x3300ff });
                break;
     ...
     */
    /**
     * @typedef {{color: v3d.Color=, specular: v3d.Color=, shininess: number=, emissive: v3d.Color=, emissiveIntensity: number=, opacity: number=, transparent: boolean=, map: v3d.Texture=}} v3dMaterialParameterPack
     */
    /**
     * @param {FBXMaterialProperties} properties
     * @param {Map<number, v3d.Texture>} textureMap
     * @param {{ID: number, relationship: string}[]} childrenRelationships
     * @returns {v3dMaterialParameterPack}
     */
    function parseParameters(properties, textureMap, childrenRelationships) {
        ...
        for (var childrenRelationshipsIndex = 0, childrenRelationshipsLength = childrenRelationships.length; childrenRelationshipsIndex < childrenRelationshipsLength; ++ childrenRelationshipsIndex) {
            ...
            switch (type) {
                ...
                case ' "EmissiveColor':
                default:
                    console.warn('v3d.FBXLoader: Unknown texture application of type %s, skipping texture.', type);
                    break;
     ...
     * @param {Map<number, {parents: {ID: number, relationship: string}[], children: {ID: number, relationship: string}[]}>} connections
     * @returns {Map<number, {map: Map<number, {FBX_ID: number, indices: number[], weights: number[], transform: number[], transformLink: number[], linkMode: string}>, array: {FBX_ID: number, indices: number[], weights: number[], transform: number[], transformLink: number[], linkMode: string}[], skeleton: v3d.Skeleton|null}>}
     */
     ...
     * @returns {{map: Map<number, {FBX_ID: number, indices: number[], weights: number[], transform: number[], transformLink: number[], linkMode: string}>, array: {FBX_ID: number, indices: number[], weights: number[], transform: number[], transformLink: number[], linkMode: string}[], skeleton: v3d.Skeleton|null}}
     */
    ...
    /**
     * Generates Buffer geometries from geometry information in FBXTree, and generates map of v3d.BufferGeometries
     * @param {{Objects: {subNodes: {Geometry: Object.<number, FBXGeometryNode}}}} FBXTree
     * @param {Map<number, {parents: {ID: number, relationship: string}[], children: {ID: number, relationship: string}[]}>} connections
     * @param {Map<number, {map: Map<number, {FBX_ID: number, indices: number[], weights: number[], transform: number[], transformLink: number[], linkMode: string}>, array: {FBX_ID: number, indices: number[], weights: number[], transform: number[], transformLink: number[], linkMode: string}[], skeleton: v3d.Skeleton|null}>} deformers
     * @returns {Map<number, v3d.BufferGeometry>}
     */
    ...
    function genGeometry(geometryNode, deformer) {
        ...
        geo.addAttribute('position', new v3d.Float32BufferAttribute(bufferInfo.vertexBuffer, 3));

        if (bufferInfo.normalBuffer.length > 0) {

            geo.addAttribute('normal', new v3d.Float32BufferAttribute(bufferInfo.normalBuffer, 3));

        }
        if (bufferInfo.uvBuffer.length > 0) {

            geo.addAttribute('uv', new v3d.Float32BufferAttribute(bufferInfo.uvBuffer, 2));

        }
        if (subNodes.LayerElementColor) {

            geo.addAttribute('color', new v3d.Float32BufferAttribute(bufferInfo.colorBuffer, 3));

        }

        if (deformer) {

            geo.addAttribute('skinIndex', new v3d.Float32BufferAttribute(bufferInfo.skinIndexBuffer, 4));

            geo.addAttribute('skinWeight', new v3d.Float32BufferAttribute(bufferInfo.skinWeightBuffer, 4));
    ...
    function parseNurbsGeometry(geometryNode) {
        ...
        if (isNaN(order)) {
            ...
            console.error('v3d.FBXLoader: Invalid Order %s given for geometry ID: %s', geometryNode.properties.Order, geometryNode.id);
            return new v3d.BufferGeometry();
     ...
     * @param {Map<number, {parents: {ID: number, relationship: string}[], children: {ID: number, relationship: string}[]}>} connections
     * @param {Map<number, {map: Map<number, {FBX_ID: number, indices: number[], weights: number[], transform: number[], transformLink: number[], linkMode: string}>, array: {FBX_ID: number, indices: number[], weights: number[], transform: number[], transformLink: number[], linkMode: string}[], skeleton: v3d.Skeleton|null}>} deformers
     * @param {Map<number, v3d.BufferGeometry>} geometryMap
     * @param {Map<number, v3d.Material>} materialMap
     * @returns {v3d.Group}
     */
    function parseScene(FBXTree, connections, deformers, geometryMap, materialMap) {

        var sceneGraph = new v3d.Group();

        var ModelNode = FBXTree.Objects.subNodes.Model;

        /**
         * @type {Array.<v3d.Object3D>}
         */
        var modelArray = [];

        /**
         * @type {Map.<number, v3d.Object3D>}
         */
        ...
        for (var nodeID in ModelNode) {
            ...
            if (! model) {
                ...
                switch (node.attrType) {
                    ...
                    case 'Mesh':
                        /**
                         * @type {?v3d.BufferGeometry}
                         */
                        var geometry = null;

                        /**
                         * @type {v3d.MultiMaterial|v3d.Material}
                         */
                        var material = null;

                        /**
                         * @type {Array.<v3d.Material>}
                         */
                        var materials = [];
                        ...
                        if ('color' in geometry.attributes) {
                            ...
                            for (var materialIndex = 0, numMaterials = materials.length; materialIndex < numMaterials; ++ materialIndex) {
                                ...
                                materials[materialIndex].vertexColors = v3d.VertexColors;
                    ...
                    case 'NurbsCurve':
                        ...
                        material = new v3d.LineBasicMaterial({ color: 0x3300ff, linewidth: 5 });
                        model = new v3d.Line(geometry, material);
                        break;
        ...
        for (var modelArrayIndex = 0, modelArrayLength = modelArray.length; modelArrayIndex < modelArrayLength; ++ modelArrayIndex) {
            ...
            if ('PreRotation' in node.properties) {
                ...
                var preRotations = new v3d.Euler().setFromVector3(parseVector3(node.properties.PreRotation).multiplyScalar(DEG2RAD), 'ZYX');
                preRotations = new v3d.Quaternion().setFromEuler(preRotations);
                var currentRotation = new v3d.Quaternion().setFromEuler(model.rotation);
        ...
        for (var FBX_ID in deformers) {
            ...
            deformer.skeleton = new v3d.Skeleton(deformer.bones);
        ...
        tmpMap.forEach(function(curveNode) {
            ...
            if (curveNode.attr === 'R') {
                ...
                if (curveNode.preRotations !== null) {
                    ...
                    var preRotations = new v3d.Euler().setFromVector3(curveNode.preRotations, 'ZYX');
                    preRotations = new v3d.Quaternion().setFromEuler(preRotations);
                    var frameRotation = new v3d.Euler();
                    var frameRotationQuaternion = new v3d.Quaternion();
     ...
     * @param {{skeleton: { bones: v3d.Bone[]}}} group
     */
    function addAnimations(group, animations) {
        ...
        for (var key in stacks) {
            ...
            group.animations.push(v3d.AnimationClip.parseAnimation(animationData, bones));
    ...
    function generateKey(animations, animationNode, bone, frame) {
        ...
        try {
            ...
            console.log('v3d.FBXLoader: ', bone);
            console.log('v3d.FBXLoader: ', error);
    ...
    function Vertex() {
         ...
         * @type {v3d.Vector3}
         */
        this.position = new v3d.Vector3();

        /**
         * Normal of the vertex
         * @type {v3d.Vector3}
         */
        this.normal = new v3d.Vector3();

        /**
         * UV coordinates of the vertex.
         * @type {v3d.Vector2}
         */
        this.uv = new v3d.Vector2();

        /**
         * Color of the vertex
         * @type {v3d.Vector3}
         */
        this.color = new v3d.Vector3();

        /**
         * Indices of the bones vertex is influenced by.
         * @type {v3d.Vector4}
         */
        this.skinIndices = new v3d.Vector4(0, 0, 0, 0);

        /**
         * Weights that each bone influences the vertex.
         * @type {v3d.Vector4}
         */
        this.skinWeights = new v3d.Vector4(0, 0, 0, 0);
    ...
    function Triangle() {
        ...
        /**
         * @type {{position: v3d.Vector3, normal: v3d.Vector3, uv: v3d.Vector2, skinIndices: v3d.Vector4, skinWeights: v3d.Vector4}[]}
         */
        this.vertices = [];
    ...
    function Face() {
        ...
        /**
         * @type {{vertices: {position: v3d.Vector3, normal: v3d.Vector3, uv: v3d.Vector2, skinIndices: v3d.Vector4, skinWeights: v3d.Vector4}[]}[]}
         */
        this.triangles = [];
    ...
    function Geometry() {
        ...
        /**
         * @type {{triangles: {vertices: {position: v3d.Vector3, normal: v3d.Vector3, uv: v3d.Vector2, skinIndices: v3d.Vector4, skinWeights: v3d.Vector4}[]}[], materialIndex: number}[]}
         */
        this.faces = [];
            ...
            switch (type) {
                ...
                case 'b':
                    ...
                    if (window.Zlib === undefined) {
                        ...
                        throw new Error('v3d.FBXLoader: External library Inflate.min.js required, obtain or import from https://github.com/imaya/zlib.js');
    ...
    function parseVector3(property) {
        ...
        return new v3d.Vector3().fromArray(property.value);
    ...
    function parseColor(property) {
        ...
        return new v3d.Color().fromArray(property.value);

    }

    function parseMatrixArray(floatString) {

        return new v3d.Matrix4().fromArray(parseFloatArray(floatString));
