...
v3d.AssimpJSONLoader = function(manager) {

    this.manager = (manager !== undefined) ? manager : v3d.DefaultLoadingManager;

};

v3d.AssimpJSONLoader.prototype = {
        ...
        loader.load(url, function(text) {
            ...
            if (typeof metadata !== 'undefined') {
                ...
                if (metadata.format !== 'assimp2json') {
                    ...
                    onError('v3d.AssimpJSONLoader: Unsupported assimp2json file format version.');
                    return;
    ...
    parse: function(json, path) {
        ...
        function parseObject(json, node, meshes, materials) {
            ...
            var obj = new v3d.Object3D(),    i, idx;

            obj.name = node.name || '';
            obj.matrix = new v3d.Matrix4().fromArray(node.transformation).transpose();
            obj.matrix.decompose(obj.position, obj.quaternion, obj.scale);

            for (i = 0; node.meshes && i < node.meshes.length; i++) {

                idx = node.meshes[i];
                obj.add(new v3d.Mesh(meshes[idx], materials[json.meshes[idx].materialindex]));
