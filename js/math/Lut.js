...
v3d.Lut = function(colormap, numberofcolors) {

    this.lut = [];
    this.map = v3d.ColorMapKeywords[colormap];
    ...
    for (var i = 0; i <= 1; i += step) {
        ...
        for (var j = 0; j < this.map.length - 1; j ++) {
            ...
            if (i >= this.map[j][0] && i < this.map[j + 1][0]) {
                ...
                var minColor = new v3d.Color(0xffffff).setHex(this.map[j][1]);
                var maxColor = new v3d.Color(0xffffff).setHex(this.map[j + 1][1]);
...
v3d.Lut.prototype = {
    ...
    changeNumberOfColors: function(numberofcolors) {
        ...
        return new v3d.Lut(this.mapname, this.n);
    ...
    addColorMap: function(colormapName, arrayOfColors) {
        ...
        v3d.ColorMapKeywords[colormapName] = arrayOfColors;
    ...
    setLegendOn: function(parameters) {
        ...
        this.map = v3d.ColorMapKeywords[this.mapname];
        ...
        for (var i = 1; i >= 0; i -= step) {
            ...
            for (var j = this.map.length - 1; j >= 0; j --) {
                ...
                if (i < this.map[j][0] && i >= this.map[j - 1][0]  ) {
                    ...
                    var minColor = new v3d.Color(0xffffff).setHex(this.map[j - 1][1]);
                    var maxColor = new v3d.Color(0xffffff).setHex(this.map[j][1]);
    ...
    setLegendPosition: function(position) {
        ...
        this.legend.position = new v3d.Vector3(position.x, position.y, position.z);
    ...
    setLegendLabels: function(parameters, callback) {
        ...
        var txtTitle = new v3d.CanvasTexture(canvasTitle);
        txtTitle.minFilter = v3d.LinearFilter;

        var spriteMaterialTitle = new v3d.SpriteMaterial({ map: txtTitle });

        var spriteTitle = new v3d.Sprite(spriteMaterialTitle);
        ...
        if (this.legend.labels.ticks > 0) {
            ...
            for (var i = 0; i < this.legend.labels.ticks; i++) {
                ...
                var txtTick = new v3d.CanvasTexture(canvasTick);
                txtTick.minFilter = v3d.LinearFilter;

                var spriteMaterialTick = new v3d.SpriteMaterial({ map: txtTick });

                var spriteTick = new v3d.Sprite(spriteMaterialTick);
