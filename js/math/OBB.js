(function() {
    ...
    const a = {
        c: null,
        // center
        u: [new v3d.Vector3(), new v3d.Vector3(), new v3d.Vector3()],
        // basis vectors
        e: [] // half width
    ...
    const yAxis = new v3d.Vector3();
    const zAxis = new v3d.Vector3();
    const v1 = new v3d.Vector3();
    const size = new v3d.Vector3();
    const closestPoint = new v3d.Vector3();
    const rotationMatrix = new v3d.Matrix3();
    const aabb = new v3d.Box3();
    const matrix = new v3d.Matrix4();
    const inverse = new v3d.Matrix4();
    const localRay = new v3d.Ray(); // OBB

    class OBB {

        constructor(center = new v3d.Vector3(), halfSize = new v3d.Vector3(), rotation = new v3d.Matrix3()) {
  ...
  */
        ...
        clampPoint(point, result) {
            ...
            const x = v3d.MathUtils.clamp(v1.dot(xAxis), - halfSize.x, halfSize.x);
            result.add(xAxis.multiplyScalar(x));
            const y = v3d.MathUtils.clamp(v1.dot(yAxis), - halfSize.y, halfSize.y);
            result.add(yAxis.multiplyScalar(y));
            const z = v3d.MathUtils.clamp(v1.dot(zAxis), - halfSize.z, halfSize.z);
