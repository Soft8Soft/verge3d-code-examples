(function() {
    ...
    const _va = new v3d.Vector3(),
        ...
        _vec = new v3d.Vector3(),
        // temporary vector
        _quat = new v3d.Quaternion(); // temporary quaternion
 ...
 * @param {boolean} estimateViewFrustum */
    ...
    function frameCorners(camera, bottomLeftCorner, bottomRightCorner, topLeftCorner, estimateViewFrustum = false) {
        ...
        if (estimateViewFrustum) {
            ...
            camera.fov = v3d.MathUtils.RAD2DEG / Math.min(1.0, camera.aspect) * Math.atan((_vec.copy(pb).sub(pa).length() + _vec.copy(pc).sub(pa).length()) / _va.length());
    ...
    v3d.CameraUtils = {};
    v3d.CameraUtils.frameCorners = frameCorners;
