...
v3d.Car = (function() {
    ...
    var frontLeftWheel = new v3d.Group();
    var frontRightWheel = new v3d.Group();
    ...
    Car.prototype = {
        ...
        update: function(delta) {
            ...
            if (controls.moveForward) {
                ...
                this.speed = v3d.Math.clamp(this.speed + delta * this.acceleration, maxSpeedReverse, this.maxSpeed);
                acceleration = v3d.Math.clamp(acceleration + delta, - 1, 1);

            }

            if (controls.moveBackward) {

                this.speed = v3d.Math.clamp(this.speed - delta * accelerationReverse, maxSpeedReverse, this.maxSpeed);
                acceleration = v3d.Math.clamp(acceleration - delta, - 1, 1);
            ...
            if (controls.moveLeft) {
                ...
                wheelOrientation = v3d.Math.clamp(wheelOrientation + delta * steeringWheelSpeed, - maxSteeringRotation, maxSteeringRotation);
            ...
            if (!(controls.moveForward || controls.moveBackward)) {
                ...
                if (this.speed > 0) {
                    ...
                    this.speed = v3d.Math.clamp(this.speed - k * delta * deceleration * brakingDeceleration, 0, this.maxSpeed);
                    acceleration = v3d.Math.clamp(acceleration - k * delta, 0, 1);

                } else {

                    var k = exponentialEaseOut(this.speed / maxSpeedReverse);

                    this.speed = v3d.Math.clamp(this.speed + k * delta * accelerationReverse * brakingDeceleration, maxSpeedReverse, 0);
                    acceleration = v3d.Math.clamp(acceleration + k * delta, - 1, 0);
            ...
            if (!(controls.moveLeft || controls.moveRight)) {
                ...
                if (wheelOrientation > 0) {
                    ...
                    wheelOrientation = v3d.Math.clamp(wheelOrientation - delta * steeringWheelSpeed, 0, maxSteeringRotation);

                } else {

                    wheelOrientation = v3d.Math.clamp(wheelOrientation + delta * steeringWheelSpeed, - maxSteeringRotation, 0);
        ...
        computeDimensions: function() {
            ...
            var bb = new v3d.Box3().setFromObject(frontLeftWheelRoot);

            var size = new v3d.Vector3();
            bb.getSize(size);
